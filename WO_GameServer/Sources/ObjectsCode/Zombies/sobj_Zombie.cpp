...
#define Super_Zombie 20299
	...
	float		_zai_IdleStatePatrolPerc = 0.3f;
	float		_zai_NoPatrolPlayerDist  = 500.0;
	float		_zai_PlayerSpawnProtect  = 150.0f;	// radius where zombie won't be spawned because of player presense
	...
	float		_zai_NoPatrolPlayerDist  = 500.0;
	float		_zai_PlayerSpawnProtect  = 150.0f;	// radius where zombie won't be spawned because of player presense
	float		_zai_PlayerSpawnProtectSZ = 5.0f;  // Super Zombie Spawn Radius && EVENT
	float		_zai_MaxSpawnDelay       = 9999.0f;
	float       _zai_AttackDamageSZ		 = 15.0f;	// Super Zombie Attack Damage
	float		_zai_AttackDamage        = 30.0f;
	int		_zai_DebugAI             = 0;
#endif
...
BOOL obj_Zombie::OnCreate()
{
	...
	//CrawlerZombie = u_GetRandom() < (1.0f / 100) ? true : false; // every 100th zombie is special
	if(HalloweenZombie) FastZombie = 1;
	//if(CrawlerZombie) FastZombie = 1;
...
bool obj_Zombie::IsPlayerDetectable(const obj_ServerPlayer* plr, float dist)
{
	...
	if(plr->loadout_->GameFlags & wiCharDataFull::GAMEFLAG_SpawnProtection || 
		plr->loadout_->GameFlags & wiCharDataFull::GAMEFLAG_NearPostBox)
		return false;
	...
	if(dist < DetectRadius) {
		AILog(1, "Zombie%p smellt player with distance %f (%f)\n", this, dist, DetectRadius);
		return true;
	}
...
GameObject* obj_Zombie::GetClosestPlayerBySenses()
{
	...
	if(found) AILog(20, "zombie%p GetClosestPlayerBySenses %s\n", this, found->userName);
	return found;
}
...
bool obj_Zombie::SenseAirHorn(const obj_ServerPlayer* plr)
{
...
bool obj_Zombie::SenseWeaponFire(const obj_ServerPlayer* plr, const ServerWeapon* wpn)
{
	...
	switch(wpn->getCategory())
	{
		default:
		case storecat_SNP:
			range = 125; // GigaToni -> Reincreated | DNC increased from 75
			break;

		case storecat_ASR:
		case storecat_SHTG:
		case storecat_MG:
			range = 100; // GigaToni -> Reincreated | DNC increased from 50
			break;

		case storecat_HG:
		case storecat_SMG:
			range = 70; // GigaToni -> Reincreated | DNC increased from 30
			break;
			
		case storecat_MELEE:
			range = 5; // sergey's design.
			break;
	}
	...
	//pitol silence
	if(wpn->m_Attachments[WPN_ATTM_MUZZLE] && wpn->m_Attachments[WPN_ATTM_MUZZLE]->m_itemID == 400156)
	{
		range *= 0.3f;
	}
	...
	if(wpn->getConfig()->m_itemID == 101088)
		range = 275; // GigaToni -> Reincreated | DNC increased from 75
...
BOOL obj_Zombie::Update()
{
	...
	switch(ZombieState)
	{
		...
		{
			if(curTime < StateStartTime + 3.0f) // wait for client "wake up" animation finish termina
				break;
		...
		case EZombieStates::ZState_Attack:
		{
			...
			if(HeroItemID == Super_Zombie)
			{
				if(attackTimer >= _zai_SZAttackTimer)
				{
					...
					{
						//if(((int)(r3dGetTime() - trg->startPlayTime_)) <= 100)
						if(trg->loadout_->GameFlags & wiCharDataFull::GAMEFLAG_SpawnProtection ||
							trg->loadout_->GameFlags & wiCharDataFull::GAMEFLAG_NearPostBox)
						{
							StopAttack();
							break;
						}
						...
						if(HeroItemID == Super_Zombie)
						{
							 damage = _zai_AttackDamageSZ;
						}else{
							 damage = _zai_AttackDamage;
						}
						// Skillsystem iKasu
						if(trg->loadout_->Stats.skillid0 == 1){
							damage -= 5.0f;
							if(trg->loadout_->Stats.skillid7 == 1)
								damage -= 5.0f;
						}
						
						// temp code for applying damage from zombie to player
						trg->loadout_->Health -= damage;
						if(HeroItemID == Super_Zombie)
						{
							...
							if(u_GetRandom(0.0f, 100.0f) < 10.0f) // chance of infecting
							{
								trg->loadout_->Toxic += 1.0f;
							}
						}
				...
				if(attackTimer >= _zai_AttackTimer)
				{
					...
					else
					{
						...
						if(HeroItemID == Super_Zombie)
						{
							if(u_GetRandom(0.0f, 100.0f) < 30.0f) // chance of infecting
							{
								trg->loadout_->Toxic += 1.0f;
							}
						}else{
...
bool obj_Zombie::ApplyDamage(GameObject* fromObj, float damage, int bodyPart, STORE_CATEGORIES damageSource)
{
	...
	if(HeroItemID == Super_Zombie)
	{
		...
		dmg *= 0.2; // dmg = 0; mudei pra valer dano no corpo do zumbi
