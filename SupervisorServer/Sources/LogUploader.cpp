...
void CLogUploader::Start()
{
	...
	//	g_api_ip->SetString("69.162.113.130");
	//	gDomainBaseUrl= "/WarZ/";
	//	gDomainPort   = 8080;
	//	gDomainUseSSL = false;
	//}
...
int CLogUploader::UploadDataToServer(CFileData& logdata, CFileData& dmpdata, CFileData& jpgdata)
{
	...
	resp = http_.SynchronousRequest(gSupervisorConfig->webAPIDomainIP_.c_str(),
		gSupervisorConfig->webAPIDomainPort_, gSupervisorConfig->webAPIDomainUseSSL_, req);
	if(resp == NULL) {
